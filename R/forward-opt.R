#' Function to search for the optimal PM design using forward selection using Mplus
  ...
  complete.var=NULL)
{
  ...
  if (max.mk==1){          # when num.miss=1
    ...
    for (num.miss in 2:max.mk){
      ...
      re.ob=list("results"=sim.results.out,"opt.design"=opt.design,"opt.pattern"=opt.pattern,"opt.probs"=opt.probs,"design.order"=design.order)
