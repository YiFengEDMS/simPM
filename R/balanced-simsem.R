...
#' Function to search for the optimal item-level PM design using lavaan/simsem
...
balance.miss.l=function(
 ...
 # multicore=T
  ...
  re.ob=list("results"=sim.results.out,"opt.design"=opt.design,"opt.pattern"=opt.pattern,
             ...
             "opt.template"=opt.template,'opt.logical'=opt.logical,'opt.output'=opt.output,"misc"=misc)
  return(re.ob)
}
